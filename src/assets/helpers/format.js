...
export function formatPhoneNumber(value) {
  ...
  const phoneNumberWithoutSeven = phoneNumber.startsWith("7")
    ? phoneNumber.slice(1)
    : phoneNumber; // Убираем начальную 7, если она есть
  ...
  if (phoneNumberLength <= 3)
    return `+7 (${phoneNumberWithoutSeven.slice(0, 3)}`;
  if (phoneNumberLength <= 6)
    return `+7 (${phoneNumberWithoutSeven.slice(0, 3)}) ${phoneNumberWithoutSeven.slice(3, 6)}`;
  if (phoneNumberLength <= 8)
    return `+7 (${phoneNumberWithoutSeven.slice(0, 3)}) ${phoneNumberWithoutSeven.slice(3, 6)}-${phoneNumberWithoutSeven.slice(6, 8)}`;
...
export function dateFormat(num, type = "year") {
  if (type === "months") {
    ...
    let months = num % 12;
    let yearsText =
      years === 1 ? "1 год" : years <= 4 ? `${years} года` : `${years} лет`;
    let monthsText =
      months === 0
        ? ""
        : months === 1
