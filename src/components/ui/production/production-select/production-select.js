...
$(document).ready(function () {
  ...
  function toggleSelect($selectBox, $selectItems) {
    ...
    if (isOpen) {
      ...
      if (autoHeight <= 310) {
        $selectBox
          .find(".production__select-items")
          .css("height", heightWithoutPadding);
  ...
  $(document).on("click", function (event) {
    ...
    if (!$target.closest(".production__select").length) {
      ...
      $(".production__select")
        .not($target.closest(".production__select"))
        .each(function () {
          const $otherSelectBox = $(this);
          const $otherSelectItems = $otherSelectBox.find(
  ...
  $(".js-apply-select").on("click", function () {
    ...
    $(".production__select--input .production__select-title").html(
      `${formatNumberWithSymbol(values.from, "")} - ${formatNumberWithSymbol(values.to)}`,
    );
