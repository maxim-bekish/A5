$(document).ready(function () {
  ...
  function updateSlider() {
    ...
    const slideWidth = Math.ceil(
      $(".team-slider .slider__slide").outerWidth(true),
    );
    ...
    function updateFirstVisibleSlide() {
      ...
      firstVisibleSlide
        .find(".slider__header .slider__image")
        .css("height", "100%"); // Устанавливаем height для первого видимого изображения
    ...
    function prevSlide() {
      if (!isAnimating) {
        ...
        const currentSlide = $sliderContainer
          .children()
          .eq(Math.abs(currentLeft / slideWidth)); // Текущий первый видимый слайд
        const nextSlide = $sliderContainer
          .children()
          .eq(Math.abs(newLeft / slideWidth));
        ...
        currentSlide.find(".slider__header .slider__image").animate(
          ...
          timeAnimate,
        ); // Плавное изменение height текущего слайда
